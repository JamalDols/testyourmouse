  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-400 font-mono text-sm">// Real-time click latency monitoring</p>
      </div>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          {!isTracking ? (
            ...
            <div className="text-center pointer-events-none">
              ...
              <p className="text-sm text-gray-500 font-mono">Click anywhere • {clickData.filter((c) => c.responseTime > 0).length} samples</p>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className="p-4 text-center bg-cyan-500/5 border-cyan-500/30">
          ...
          <p className="text-2xl text-cyan-400 font-mono tabular-nums">{avgResponseTime || "---"}</p>
        ...
        <Card className="p-4 text-center bg-green-500/5 border-green-500/30">
          ...
          <p className="text-2xl text-green-400 font-mono tabular-nums">{minResponseTime === Infinity ? "---" : minResponseTime}</p>
          <p className="text-xs text-gray-500 font-mono">ms</p>
        </Card>
        ...
        <Card className="p-4 text-center bg-red-500/5 border-red-500/30">
          ...
          <p className="text-2xl text-red-400 font-mono tabular-nums">{maxResponseTime || "---"}</p>
          <p className="text-xs text-gray-500 font-mono">ms</p>
        </Card>
        ...
        <Card className="p-4 text-center bg-purple-500/5 border-purple-500/30">
          ...
          <p className="text-2xl text-purple-400 font-mono tabular-nums">{clickData.filter((c) => c.responseTime > 0).length}</p>
        </Card>
      </div>
        ...
        <Card className="p-6 bg-[#12121a] border-cyan-500/30">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={chartData}>
              ...
              <XAxis dataKey="index" stroke="#8b8b9a" style={{ fontSize: "12px", fontFamily: "monospace" }} label={{ value: "Click #", position: "insideBottom", offset: -5, fill: "#8b8b9a" }} />
              <YAxis stroke="#8b8b9a" style={{ fontSize: "12px", fontFamily: "monospace" }} label={{ value: "ms", angle: -90, position: "insideLeft", fill: "#8b8b9a" }} />
              <Tooltip
                contentStyle={{
              ...
              />
              <Area type="monotone" dataKey="time" stroke="#00d9ff" strokeWidth={2} fill="url(#colorTime)" />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
      )}
        ...
        <Card className="p-4 bg-[#12121a] border-cyan-500/30">
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {clickData
              ...
              .reverse()
              .map((click) => (
                <div key={click.id} className="flex items-center justify-between p-2 rounded border border-cyan-500/20 bg-cyan-500/5">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      ...
                      className={`font-mono uppercase ${
                        click.clickType === "left" ? "border-cyan-500/30 text-cyan-400" : click.clickType === "right" ? "border-purple-500/30 text-purple-400" : "border-green-500/30 text-green-400"
                      }`}
                    >
                      {click.clickType}
                    </Badge>
                    <span className="text-lg font-mono tabular-nums" style={{ color: getColorForResponseTime(click.responseTime) }}>
