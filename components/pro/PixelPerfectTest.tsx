  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="text-center mb-6">
        ...
        <p className="text-gray-400 font-mono text-sm">// Click all targets as accurately as possible</p>
      </div>
      ...
      {!gameStarted ? (
            ...
            {(["easy", "medium", "hard"] as const).map((level) => (
              <button
                ...
                className={`px-6 py-3 rounded-lg font-mono transition-all ${
                  difficulty === level ? "bg-cyan-500/20 text-cyan-400 border-2 border-cyan-500" : "bg-cyan-500/5 border border-cyan-500/30 text-gray-400 hover:bg-cyan-500/10"
                }`}
              >
                [{level.toUpperCase()}]
              </button>
        ...
        <>
          ...
          <Card className="relative h-[500px] overflow-hidden bg-[#0a0a0f] border-2 border-cyan-500/30 cursor-crosshair" onClick={handleMiss}>
            {/* Grid pattern */}
            <div
            ...
            {remaining === 0 && (
              <div className="absolute inset-0 flex items-center justify-center bg-[#0a0a0f]/90 backdrop-blur-sm">
                <div className="text-center">
                  ...
                  <p className="text-xl text-cyan-400 font-mono mb-6">ACCURACY: {accuracy}%</p>
                  <button onClick={reset} className="px-6 py-3 bg-cyan-500/20 hover:bg-cyan-500/30 border border-cyan-500/50 text-cyan-400 rounded-lg transition-all font-mono tracking-wider">
                    [TRY_AGAIN]
                  </button>
                </div>
              </div>
