            ...
            <div className="absolute inset-0 pointer-events-none z-20">
              <div
                className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 rounded-full animate-ping ${
                  activeButton === "left" ? "bg-cyan-500/40 glow-cyan" : activeButton === "right" ? "bg-purple-500/40 glow-purple" : "bg-green-500/40 glow-green"
                }`}
              />
            </div>
          )}
              ...
              {clickLog.map((event) => (
                <div
                  ...
                  className={`flex items-center justify-between p-2 rounded border ${
                    event.button === "left" ? "bg-cyan-500/5 border-cyan-500/30" : event.button === "right" ? "bg-purple-500/5 border-purple-500/30" : "bg-green-500/5 border-green-500/30"
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <Circle className={`w-3 h-3 fill-current ${event.button === "left" ? "text-cyan-500" : event.button === "right" ? "text-purple-500" : "text-green-500"}`} />
