  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <Card className="p-8 text-center bg-[#12121a] border-cyan-500/30">
        ...
        <p className="text-gray-400 mb-6 font-mono text-sm">// Click individuales detectados automáticamente</p>
      ...
      <Card className="p-4 bg-[#12121a] border-cyan-500/30">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={reset} className="px-4 py-2 bg-cyan-500/10 hover:bg-cyan-500/20 border border-cyan-500/30 rounded-lg transition-colors text-sm text-cyan-400 tracking-wider">
            [RESET]
          </button>
        </div>

        {results.length === 0 ? (
          <p className="text-sm text-gray-500 text-center py-8 font-mono">// WAITING_FOR_INPUT</p>
        ) : (
          <div className="space-y-2">
            {results.map((result) => (
              <div
                key={result.id}
                className={`flex items-center justify-between p-3 rounded-lg border ${result.isDoubleClick ? "bg-red-500/5 border-red-500/30" : "bg-green-500/5 border-green-500/30"}`}
              >
                <div className="flex items-center gap-3">
                  {result.isDoubleClick ? <AlertCircle className="w-5 h-5 text-red-500 animate-pulse" /> : <CheckCircle2 className="w-5 h-5 text-green-500" />}
                  <div>
                    <p className="text-sm font-mono tracking-wide">{result.isDoubleClick ? "[DOUBLE_CLICK]" : "[SINGLE_CLICK]"}</p>
                    {result.interval > 0 && <p className="text-xs text-gray-500 font-mono">Δt: {result.interval}ms</p>}
                  </div>
                </div>
