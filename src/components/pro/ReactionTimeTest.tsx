...
export function ReactionTimeTest() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card 
        className={`p-8 text-center h-96 flex items-center justify-center cursor-pointer transition-all ${getBackgroundColor()}`}
        onClick={testState === 'idle' ? startTest : handleClick}
      >
        <div>
          <Zap className={`w-24 h-24 mx-auto mb-4 ${
            testState === 'ready' ? 'text-green-400 animate-pulse' : 
            testState === 'waiting' ? 'text-red-400' :
            testState === 'too-early' ? 'text-yellow-400' :
            'text-cyan-400'
          }`} />
              ...
              {[0, 1, 2, 3, 4].map(i => (
                <div 
                  ...
                  className={`w-2 h-2 rounded-full ${
                    i < Math.floor(countdown / 2) ? 'bg-red-500' : 'bg-red-500/20'
                  }`}
                />
              ))}
            </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card className="p-4 text-center bg-cyan-500/5 border-cyan-500/30">
          ...
          <p className="text-3xl text-cyan-400 font-mono tabular-nums">
        ...
        <Card className="p-4 text-center bg-purple-500/5 border-purple-500/30">
          ...
          <p className="text-3xl text-purple-400 font-mono tabular-nums">
        ...
        <Card className="p-4 text-center bg-green-500/5 border-green-500/30">
          ...
          <p className="text-3xl text-green-400 font-mono tabular-nums">
            {bestTime ? bestTime : '---'}
          </p>
          <p className="text-xs text-gray-500 font-mono">ms</p>
        </Card>
            ...
            {results.map((time, index) => (
              <Badge 
                ...
                className={`text-center py-2 font-mono tabular-nums ${
                  time === bestTime ? 'bg-green-500/10 border-green-500/50 text-green-400' :
                  time < 200 ? 'bg-cyan-500/10 border-cyan-500/50 text-cyan-400' :
                  time < 300 ? 'bg-purple-500/10 border-purple-500/50 text-purple-400' :
